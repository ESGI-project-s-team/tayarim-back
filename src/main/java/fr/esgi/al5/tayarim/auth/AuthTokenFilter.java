...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        mapOfExcludeMethodPath.put("POST", List.of("/auth/login", "/auth/refresh", "/reservations"));
        mapOfExcludeMethodPath.put("PUT", List.of("/reservations/paymentIntent/\\d+"));
        mapOfExcludeMethodPath.put("GET", List.of("/logements/types", "/reglesLogement", "/amenagements"));
        ...
        return
                !method.isBlank() &&
                        excludePaths.stream().anyMatch(pattern -> Pattern.compile(pattern).matcher(path).matches());
    }
}